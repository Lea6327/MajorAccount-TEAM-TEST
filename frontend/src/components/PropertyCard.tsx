...
// Validation regex for volume/folio inputs
...
export default function PropertyCard({ initial }: { initial: InternalProperty }) {
  ...
  // Status is recomputed when volume/folio values change
  const nextStatus = useMemo(
    () => (vol && fol ? "KnownVolFol" : "UnknownVolFol") as InternalProperty["status"],
    [vol, fol]
  );

  // Confirm button handler with inline validation
  const onConfirm = () => {
    const e: typeof errs = {};
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            ...
            <div className="text-base text-gray-700">
              <span className="font-medium">Lot/Plan:</span> {p.lotPlan?.lot ?? "-"} / {p.lotPlan?.plan ?? "-"}
            </div>
            ...
            <div className="text-sm text-gray-500">
              <span className="font-medium">Source:</span> {p.sourceTrace.provider} · {p.sourceTrace.requestId} · {p.sourceTrace.receivedAt}
            </div>
          </div>
